...
ec_err_t ec_method_blake2b_512_hash(unsigned char hash[EC_METHOD_BLAKE2B_512_BYTES], ec_cert_t *c) {
  ...
  if(!c->records)
    return EC_EMISSING;
  assert(EC_METHOD_BLAKE2B_512_BYTES >= crypto_generichash_BYTES_MIN, EC_ESIZE, NULL);
  assert(EC_METHOD_BLAKE2B_512_BYTES <= crypto_generichash_BYTES_MAX, EC_ESIZE, NULL);
